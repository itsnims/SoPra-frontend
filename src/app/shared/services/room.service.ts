...
export class RoomService {
  ...
  roomLogin(room: Room): Observable<Room> {
    ...
    const bodyString = JSON.stringify({name: room.name, owner: this.showCurrentHost(), maxplayer: room.maxPlayers, pathname: room.pathname, password : room.pwd}); // TODO add path ID
    // const userID = JSON.stringify({id: user.id})
